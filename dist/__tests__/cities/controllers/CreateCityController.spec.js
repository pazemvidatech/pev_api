...
describe('CreateCityController', () => {
  beforeAll(async () => {
    ...
    await factory.connection.query(`INSERT INTO STATES(id, name, uf, created_at, updated_at )
          values('${stateId}', 'Minas Gerais', 'MG', 'now()', 'now()')`);
  ...
  it('should be able to create a new city', async () => {
    ...
    await factory.app.post(route).send({
      ...
      expect(res.body.name).toBe('Viçosa');
      expect(res.body.state_id).toBe(stateId);
    });
  });
  it('should not be able to create a new city with same name', async () => {
    const {
      body
    } = await factory.app.post('/v1/sessions').send({
      email,
    ...
    await factory.app.post(route).send({
      ...
      expect(res.body.message).toBe('This city already exists');
