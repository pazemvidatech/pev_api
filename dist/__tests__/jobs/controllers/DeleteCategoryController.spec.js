...
describe('DeleteCategoryController', () => {
  ...
  it('should be able to delete a specific category', async () => {
    ...
    await factory.app.del(route + `/${category_id}`).set({
      Authorization: `Bearer ${token}`
    }).expect(204).then(async () => {
      const category = await factory.connection.query(`SELECT * FROM categories WHERE id = '${category_id}'`);
      expect(category.length).toBe(0);
    });
  });
  it('should not be able to delete category that does not exists', async () => {
    const {
      body
    } = await factory.app.post('/v1/sessions').send({
      email,
